...
async function runGenerator(blueprint, options) {
  ...
  let generatorMetaPath = path.resolve(
    process.cwd(),
    "./codotype-generator.js"
  ); // TODO - constantize MAGIC STRING

  if (!fs.existsSync(generatorMetaPath)) {
    console.log("NOT FOUND - check for JSON");
    generatorMetaPath = path.resolve(
      process.cwd(),
