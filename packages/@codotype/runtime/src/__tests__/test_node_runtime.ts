...
describe("testing @codotype/runtime v2", () => {
    ...
    test("testing @codotype/runtime composeWith", async () => {
        ...
        const txtExists = fs.existsSync(
            resolve(cwd, ".codotype-out/test-project/project_name/Hello.txt"),
        );
        const readmeExists = fs.existsSync(
            resolve(
                cwd,
                ".codotype-out/test-project/project_name/README-TEST.md",
            ),
        );
        ...
        const scopedFile = fs.existsSync(
            resolve(
                cwd,
                ".codotype-out/test-project/project_name/scoped-output/SCOPED-TEST.md",
            ),
        );
