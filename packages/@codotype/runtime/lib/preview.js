...
module.exports = class CodotypeRuntime {
  ...
  async execute ({ build }) {
    ...
    const dest = path.join(output_directory, blueprint.identifier);
    ...
    const destRoot = path.join(this.options.cwd, OUTPUT_DIRECTORY, output_directory, build.blueprint.identifier);
    const manifestDest = path.join(destRoot);
    fs.writeFileSync(path.join(manifestDest + '/codotype-build-preview.json'), JSON.stringify(this.buildObj, null, 2))
  ...
  writeFile (dest, compiledTemplate) {
    return new Promise((resolve, reject) => {
      this.buildObj[dest] = compiledTemplate;
      return resolve();
    })
  }
