...
const testCases: [string, AttributeInput, Attribute[], string | null][] = [
    ["valid attr (empty collection)", { ...attributeExample01 }, [], null],
    [
...
describe("validateAttribute", () => {
    testCases.forEach(testCase => {
        test(testCase[0], () => {
            const expectedResult: string | null = validateAttribute({
                attributeInput: testCase[1],
                attributeCollection: testCase[2],
            });
            expect(expectedResult).toBe(testCase[3]);
        });
    });
});
